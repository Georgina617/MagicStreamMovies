...
import (
	...
	"github.com/Georgina617/MagicStreamMovies/Server/MagicStreamMoviesServer/models"
...
if err != nil {
    // Fallback to local ranking if AI fails
    sentiment, rankVal = LocalRanking(req.AdminReview)
}
...
func GetReviewRanking(adminReview string,client*mongo.Client,c*gin.Context) (string, int, error) {
	// Validate input
	adminReview = strings.TrimSpace(adminReview)
	if adminReview == "" {
		return "", 0, errors.New("review text cannot be empty")
	}
	...
	// Build comma-separated ranking names (excluding value 999)
	...
	sentimentDelimited := strings.Join(rankingNames, ", ")

	// Load env (ignore if already loaded in main)
	_ = godotenv.Load(".env") 

	apiKey := os.Getenv("OPENAI_API_KEY")
	if apiKey == "" {
		return "", 0, errors.New("OPENAI_API_KEY is missing")
	}
	...
	if basePrompt == "" {
		return "", 0, errors.New("BASE_PROMPT_TEMPLATE not set in environment")
	}
	...
	log.Println("AI Raw Response:", response)

	// Match with DB ranking list
	var rankVal int
	for _, ranking := range rankings {
		if strings.EqualFold(ranking.RankingName, response) { // case-insensitive
	...
	if rankVal == 0 {
		return "", 0, fmt.Errorf("AI returned unknown ranking: %s", response)
	}
...
func LocalRanking(review string) (string, int) {
	reviewLower := strings.ToLower(review)

	positiveWords := []string{"good", "great", "excellent", "love", "amazing", "helpful", "improve", "well", "nice"}
	negativeWords := []string{"bad", "poor", "terrible", "hate", "worse", "worst", "disappoint", "slow", "not good"}

	score := 0

	for _, word := range positiveWords {
		if strings.Contains(reviewLower, word) {
			score++
		}
	}
